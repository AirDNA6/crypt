...
import HallFourMovieThumb from './HallFourMovieThumb'
import HallFourColGrid from './HallFourColGrid'
...
class HallFour extends Component {
  state = {
    movies: [],
    heroImage: null,
    loading: false,
  ...
  componentDidMount() {
    ...
    const endpoint = `${API_URL}movie/now_playing?api_key=${API_KEY}&language=en-US&with_genres=53&page=1`
  ...
  render() {
    ...
    <Navbar.Brand style={{ backgroundColor: "#21d192", color: "#343A40"}}>HALL 4</Navbar.Brand>
  </Navbar>
        ...
        header={`Thrilller Movies`}
            loading={this.state.loading}
        //    vote_count={this.state.vote_count}
            >
            {this.state.movies.map ( (element, i) => {
              return <HallFourMovieThumb
                        key={i}
                        clickable={true}
